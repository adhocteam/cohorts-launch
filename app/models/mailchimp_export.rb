...
class MailchimpExport < ActiveRecord::Base
  ...
  private
    ...
    def send_to_mailchimp
      ...
      # new_static_segment_id = Gibbon.listStaticSegmentAdd(id: Logan::Application.config.cohorts_mailchimp_list_id, name: name)

      # add the email addresses to the new static segment
      # resp = Gibbon.listStaticSegmentMembersAdd(id: Logan::Application.config.cohorts_mailchimp_list_id, seg_id: new_static_segment_id, batch: recipients)

      # New API 3.0 version:
      gibbon = Gibbon::Request.new
      resp = gibbon.lists(Logan::Application.config.cohorts_mailchimp_list_id).segments.create(body: { name: name, static_segment: recipients })
