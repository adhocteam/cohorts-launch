...
# Thanks michaelglass: https://gist.github.com/michaelglass/8610317
module AlertConfirmer
  ...
  private
    ...
    def handle_js_modal(name, return_val, wait_for_call = false)
      ...
      if wait_for_call
        timed_out = false
        timeout_after = Time.zone.now + Capybara.default_wait_time
        loop do
          ...
          break if page.evaluate_script(modal_called) ||
                   (timed_out = Time.zone.now > timeout_after)
